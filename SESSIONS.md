...
- **InternVL3 bake-off:** user asked to run it; host torch is CPU-only so it runs in the embed container. Hit two dep walls (transformers `all_tied_weights_keys`; bnb↔CUDA13/triton), retried bf16+CPU-offload — running at session note time. 16 GB model cached in data/hf-cache.
