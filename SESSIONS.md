...
**Left:** **2 user settings changes** (no redeploy): Space hardware → **CPU basic**; fix `BACKEND_URL` typo `.loca.it`→`.loca.lt`. Then I live-verify RUNNING + chat. Live deploy stays user-gated (classifier). Then S46 in-Space engine; S44b artifact upload pending.
