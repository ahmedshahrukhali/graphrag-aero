...
- **First deploy build-errored → fixed.** User ran the deploy (HfApi upload OK, Space commit `be3c428`) but the gradio build FAILED: HF mounts ONLY the root `requirements.txt` (`source=requirements.txt`), so our nested `-r hf_space/requirements.txt` couldn't resolve (`No such file: /tmp/hf_space/requirements.txt`). The plan's `-r` single-source mechanism doesn't survive HF's build mount. **Fix:** deploy script now FLATTENS the `-r` at stage time (expands into `hf_space/requirements.txt` contents) → self-contained deploy file, still single-sourced.
- **Then a RUNTIME error (key learning):** with the flatten fix the build passed and the app started (`Running on local URL :7860`) but ZeroGPU killed it — **`No @spaces.GPU function detected during startup`**. ZeroGPU hardware REQUIRES a `@spaces.GPU` function; the thin shell has none (the engine is S46/S48). **Thin-shell-on-ZeroGPU is impossible.** Fix = run the thin shell on **CPU basic**, flip to ZeroGPU at S48 when the GPU engine ships. (No code/redeploy change — just the hardware setting.) torch==2.11.0 was already CACHED in HF's ZeroGPU base layer, so it was never the build issue.
- Confirmed S46 inputs while reading: `graph_context.json`=`{occ_id:row}`, `cites_edges.json`=`{occ_cites,reg_occs,occ_url}`; backend `/graph/{doc_id}` returns one prefix-stripped `graph_context` row (→ `doc_lookup` shape). Existing recurring-context fake-driver tests only cover the Python post-processing, not the Cypher graph logic — the port's tests will add that coverage. (S46 NOT started — user paused it.)
- **✅ Demo restored + live-verified.** User switched Space hardware → `cpu-basic` and fixed `BACKEND_URL`. Then UI was unresponsive = backend tunnel 503: all 7 Docker containers were UP but the pm2 `localtunnel`+`wake-proxy` were STOPPED → started both (host processes, not Docker; wake-proxy S44 hardening now took effect on fresh start). Live-verified: Space `RUNNING` cpu-basic / `/config` 75 comps / gradio 5.49.1; tunnel `/healthz` all-green; `POST /query/stream` streams `status`→`sources` (a03q0109 "FUEL EXHAUSTION", page+bbox) end-to-end. S45 = DONE.
**Left:** Demo relies on the host tunnel staying up (pm2 localtunnel+wake-proxy). **Next: S46** in-Space engine (`graph_local.py` + `zerogpu_engine.py`, offline; artifact shapes confirmed) → S47 toggle/fallback → S48 ZeroGPU deploy (adds `@spaces.GPU` → flip hardware back to ZeroGPU). S44b artifact upload (HF_TOKEN) pending. Live public deploy stays user-gated (classifier).
...
**Achieved:**
- Space 503 = both pm2 apps stopped — `pm2 start`, live-verified public `/healthz` 200. New `scripts/tray/` AHK v2 tray controller (pm2 + docker compose, hidden terminal, 15s poll, Exit = stop stack); live-verified partial→on detection; left running green.
...
- S40 visual check surfaced a real bug → **S41** (open, UNDIAGNOSED): "runway excursion" renders cited pages with zero highlight boxes. I theorized "front-matter" root cause twice and was WRONG — user screenshot showed "Runway Excursion" IS on the cited p.2 pages, unboxed. Bug is in the draw path, not page selection. Stop theorizing; next session must run a live pdfplumber `page.search` check first.
**Left:** S41 — live-diagnose the highlight draw path (see MANIFEST pointer for exact first check). No fix direction yet; do not guess.
...
**Achieved:**
- §1: hybrid re-embed complete — 77,173 chunks, dense+sparse (153,845 indexed vectors). A/B eval: dense R@5=0.727 MRR=0.682 nDCG@5=0.694 (n=11). Hybrid identical — sparse fires (50+50→82 fused); parity at n=11 is expected (reranker normalises).
- §2: `agent/reformulate.py` — real hop-N query expansion (TF-weighted novel tokens)
- §3: HITL removed; `agent/feedback.py` FeedbackStore (`unaccepted_qa`); `excluded_chunk_hashes` + `rejected_prior` in state; `POST /reject/{thread_id}`, `POST /resolve/{rejection_id}`; top-N fix (only top-3 reranked chunks excluded, not all candidates); `query_emb` stored in checkpoint
- §4-prep: Aircraft entity extraction (Bombardier-first + competitors), `Aircraft` node + `:INVOLVES` edge activated; 16 new tests; 505 passed
**Left:** §4 graph schema restructuring — verify corpus doc_ids, wire `Finding→Recommendation→IMPLEMENTS→Regulation→CITES→AC` traversal depth in `graph/query.py`
