...
## Resume pointer
**⮕ CURRENT (S50 code ☑ by fable-5, 2026-06-12): reverse fallback backend→in-Space engine — AWAITING USER DEPLOY.** `hf_space/engine_router.py` routes each Ask turn with two-way fallback; the host pm2 tunnel is no longer load-bearing for the public demo. Next step: user runs `scripts\deploy_space.ps1`, then verify by asking with the 🚀 toggle OFF while the tunnel is down (expect "Backend unreachable, falling back to in-Space engine…" then a full answer). S49/S50 detail in the active-track block below.
...
**☑ S49 DONE (ZeroGPU deps hotfix, fable-5, 2026-06-12).** First live offline query crashed `ModuleNotFoundError: qdrant_client` — S48 deployed the engine but skipped BOTH halves of its own plan: the heavy pip block was never appended (`space_root/requirements.txt` still read "S48 — NOT yet") and the deploy whitelist never staged the engine's first-party imports (`embed/`, `retrieve/`, `agent/` — the next crash in line). Fix: heavy block appended (qdrant-client, FlagEmbedding, transformers>=4.51,<5 — Qwen3 floor / BGE-M3 ceiling — accelerate; torch+spaces are ZeroGPU-injected, never pinned); `deploy_space.ps1` stages an explicit 12-file import closure (`embed/{__init__,bge_m3,ids,jsonl,qdrant}`, `retrieve/{__init__,pipeline,reranker,search}`, `agent/{__init__,prompts,state}` — whole dirs still never shipped). Also repaired `.gitignore`'s `data/space_index/` line (appended as UTF-16, NUL bytes → dead pattern; rewritten UTF-8, 764 MB artifact dir now ignored). User redeployed → **offline generation verified live. ZeroGPU active track (S44–S49) COMPLETE.**
**☑ S50 code DONE (reverse fallback, fable-5, 2026-06-12) — deploy pending.** Toggle-OFF (or quota-burned) queries died raw when the pm2 tunnel/backend was down — the host tunnel was load-bearing for the public demo. New `hf_space/engine_router.py` (gradio-free so it tests offline): `stream_with_fallback` keeps the S47 quota fallback (in-Space→backend) and adds the reverse — backend `ApiError` 5xx **before the first event** → "Backend unreachable, falling back to in-Space engine…" status + in-Space answer. Mid-stream drops and 4xx still raise (no event replay into the transcript); quota-burned + backend-down = friendly dead-end ApiError instead of a traceback. `app.py`'s `_do_stream` closure replaced by the router call. 11 new tests (run WITHOUT gradio, unlike the app helpers) — hf_space suite 85 passed, 1 skipped. Residual: quota+tunnel both down stays a dead end (nothing left to answer with).
